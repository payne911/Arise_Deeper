...
// Used a static import because SUBDIVISIONS shows up so often...

/**
 * Subcell visibility system, tracking light using {@link FieldOfView} but with a larger grid and vision radius to smooth out blocks.
 * <br>
...
public class SightSystem {
    ...
    /**
     * The latest visibility grid, as a 2D double array with larger dimensions than the map by a factor of the subdivision amount.
     */
    ...
    public void updateLighting(BaseMapLayer currLevel, int player_X, int player_Y, int sightRangeRadius) {
        ...
        // Since resistance and visible use a larger map size, we offset the player position into the center subcell of that larger map.
        // {@code player_X * SUBDIVISIONS} would put the player on an edge subcell; adding {@code SUBDIVISIONS / 2} puts him in the middle.
        // sight radius must also be multiplied by SUBDIVISIONS to see the correct distance. There may be quirks with the actual radius because
        // of how FieldOfView considers radius 1 to only include the player (I think), but radius 5 (for SUBDIVISIONS == 5) would see halfway
        // into the next adjacent cell. A possible tweak would be to give as the last parameter: {@code sightRangeRadius * SUBDIVISIONS - SUBDIVISIONS / 2}.
